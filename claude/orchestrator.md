...
2. **Give the stop a minute or two before deciding it failed.** `stop` is asynchronous and (per #103) **cannot interrupt an in-flight LLM call** — the loop only honors the signal at the next round boundary. So a new round firing immediately after the request, or `status` still reading `working` for a few seconds, is **expected, not proof the stop failed** — especially when rounds are long (a slow prefill can be 30–70s, so one boundary is over a minute away). Wait ~1–2 minutes (a couple of round-boundaries) and watch the `request fired` count: if it *stops climbing*, the stop took — be patient. Only if rounds keep firing past that window is the stop genuinely not being honored.
3. **If stop still doesn't work after that window** (daemon wedged, ghost-running state, rounds still climbing): fall back to `pkill -x taskd` to kill the daemon. The task survives in the DB and can be resumed once the daemon comes back up. Killing the daemon is more disruptive (leaves stale `working` ghost state to clear, unloads the model) — it's the fallback, not the reflex.
4. **Avoid `task <id> abort` for runaway recovery.** Abort has the same blast radius as stop but additionally makes the task unrecoverable — you lose the ability to resume from where it was. Only use abort when you actually want the task gone permanently (e.g. you've decided not to pursue this work at all). **Always get the user's explicit permission before aborting a task — it is irreversible, and a vague instruction like "clean it up" is NOT authorization to abort.** Clean the branch / bookmark / workspace freely; confirm before the abort itself.
...
**Declare dependencies explicitly.** When tickets or subtasks have an ordering — one builds on another's merged code or convention, or simply needs to ship first — proactively use the `dependencies` command (`task <id> dependencies add <ref>`, or `--deps` at `subtask add`) to declare and track it rather than holding the order only in your head. A declared dependency hard-blocks `move build` until the blocker is `Done` — planning ahead is allowed, only the build waits on the dependency — so the ordering is visible in the task graph and survives context loss instead of living implicitly in your dispatch sequence.
...
**Mode selection.** Tasks have a mode (`auto | manual`) set at `task create --mode ...`, or globally via `task mode auto|manual`, or per-task via `task <id> mode`. Pick by who's writing plan/code:

- **Auto mode** — harness planner/coder agents do the work. You stay at gates doing orchestrator-direct reviews. Cross-machine default; closest to the original flow.
- **Manual mode** — Claude (you in this session) owns the *plan*: writes it directly to the seeded path, and the harness reviews it without dispatching a planner agent that would fight your draft in cycles. The implementation phase follows the normal Coder-dispatch pattern — manual mode is about avoiding planner conflicts on the plan, not about the orchestrator personally writing code. You drive transitions with `review run --follow` and `approve`; you dispatch Coders for build-phase work as usual. The right mode when you have specific knowledge to inject (architectural constraints, prior decisions, file-level context the harness planner can't know).

Install ships `auto`. If you find yourself pre-populating plans for every task, switch the global default once: `task mode manual`.

**Default flow under each mode:**

- **Auto:** `task create --code ...` → `task X move plan` (planner agent dispatched) → harness review cycles → at the plan_approval gate, run orchestrator-direct reviewer → `task X approve` → `task X move build` (builder agent dispatched) → harness review cycles → at the build_approval gate, run orchestrator-direct reviewer → `task X approve`.
- **Manual:** `task create --code --mode manual ...` → write plan to seeded path → `task X review run --follow` (harness review cycles only, no agent writes the plan) → iterate or `task X approve` → `task X move build` (creates workspace + branch, no agent) → commit code on the task branch → `task X review run --follow` → iterate or `task X approve` → done. The gate-time orchestrator review IS this Claude session writing the code in manual mode; the harness reviewers play the "second pair of eyes."

In both modes:
- `task X reject` drops back from a review state. Feedback required in auto (planner's input for next iteration), optional in manual (audit-log artifact only).
- `task X move done --force` closes a task completed outside the harness. It lands `Done(Complete)` from ANY non-terminal phase — plan, plan_review, ready, build, or paused (`from_phase` records where it closed from) — there is NO "build never ran" guard, so it works even straight from `plan`. It rejects (409) only when an active failing review-kickback is pending; `reject`/`resume` that first. The bare `move done` (no `--force`) errors with a redirect that offers both `--force` (complete) and `abort` (cancel) — that message is the only place "abort" enters the picture, NOT a behavior of the `--force` path itself. Task row is preserved for history; abort is the destructive cancel verb.
- **Forcing a manual task into build — TINY changes only.** There is no `--force` into `build` (unlike `move done --force`): reaching `build` requires the plan-approval gate, which only opens after a plan-review *cycle* actually runs (the gate needs `activity=Paused` with a verdict — `move plan_review` lands at `PlanReview/Idle`, which is NOT a gate, and `approve` refuses there). The escape hatch is `task X move ready` (transitions `plan → ready` directly, bypassing the gate) then `task X move build`. **Reserve this for genuinely trivial changes** — a one-file doc/README, a typo fix — where there is nothing a plan review would catch. For anything substantive, run the plan review (`review run`); skipping the gate on real work just defers plan-level problems into code thrash, which is exactly what the gate exists to prevent.
- `task X review` (bare) is the read-only phase-aware lens — TTY shows prose, non-TTY emits JSON to stdout + prose to stderr. Exit code 0 always.
- `task X review run --follow|--wait` triggers + observes. Exit codes: 0 approved, 11 revise, 2 halted, 10 question.

**Subtask decomposition — manual parent / auto subtasks.** For work worth decomposing, the high-leverage shape is a MANUAL parent with AUTO subtasks: you author the decomposition + the subtasks (`task <id> subtask add --title … --deps <proj>#<root>.<seq>`), gate each subtask's plan and build, and merge; each AUTO subtask runs the cheap local model (qwen) to plan/build/self-review its own slice — the expensive model judges, the cheap model writes. Crux: child dispatch is mode-INDEPENDENT (`move plan`/`move build` on a subtask IS the dispatch trigger and fires regardless of the parent's mode) but, since #98, phase-GATED: a child's `move build` is refused until the parent's decomposition plan is approved (parent `ready`+; **Gate 1**), and the parent can't move forward out of `plan` until every subtask has reached `ready` (**Gate 2**). The resulting flow: parent plans the decomposition → each subtask plans and reaches `ready` → the parent's decomposition is approved (advancing it to `ready`, which opens Gate 1) → subtasks build in dependency order. A subtask inherits the parent's mode at creation (a manual parent yields manual-labelled children); they must run AUTO — the harness now resets a child to Auto at creation, but if one still shows manual, `task <id>.<seq> mode auto` before `move`. Close the decomposition parent (whose own *build* never runs — its `plan` phase IS the decomposition, now gate-reviewed) with `task <parent> move done --force` once its subtasks are done — `--force` lands `Done(Complete)` from any non-terminal phase, so the parent's unbuilt state is no obstacle. **Poll subtasks; don't trust silence** — a harness/qwen subtask runs in the daemon, not your session, and its completion does NOT re-invoke you; poll `task <id> status` + the subtask's `events.jsonl` freshness on a cadence to tell working from hung.

**Dependent subtasks plan early but build on their dependency, not the parent.** Subtasks needn't run in parallel — sequential is fine. Since #98 a declared dependency gates only BUILD, not plan: a dependent subtask B can plan ahead while its dependency A is still in flight (so all subtasks reach `ready` up front, which is what lets the parent leave `plan`), but B's BUILD waits for A to be `done`. Because every subtask workspace commits into the same colocated repo, B's build should start from A's resulting commit — set B's workspace base to A, or tell the Coder to pull A's commit in — not from the shared parent base, or B can't see A's work.
...
**Sensitive surface = always review at both gates, regardless of cycle count.** Sensitive surface = compaction, LLM provider, governor, agent lifecycle (phase/build, phase/planning, review_cycle), data persistence (storage layer, schema migrations), or anything where a wrong default has cross-cutting downstream effects. Converging plan_review cycles are not evidence the plan is correct — a plan that converged in 1-2 clean cycles can still hide the kinds of issues only the orchestrator-direct reviewer catches: a pass-by-luck literal assertion that survives a rename, a contract violation between paired functions, a dead enum variant whose backing logic was never written. Skipping the build-gate review on a sensitive surface is how a critical bug ships unnoticed.

**A verification step that mutates state must isolate that state — and a live schema migration is applied under supervision, not by a Coder's "does it run" check.** A Coder told to "verify the migration runs cleanly" can run the daemon against the *default* data dir and silently migrate the live DB (a schema change on real data) while reporting "live DB untouched" — the migration being sound is luck, not safety; it is the footgun. Any daemon/migration verification must point at a disposable `XDG_DATA_HOME`/`XDG_STATE_HOME`, never the real one; state that isolation explicitly in the Coder brief for migration tickets. Keep the LIVE application of a schema migration a supervised, backed-up step (build+test on a bookmark; back up the DB; apply; verify row counts + `foreign_key_check`) — a different risk class from shipping code, and one to confirm with the user rather than fold into autonomous dispatch.

**Skip orchestrator-direct review only for super-small changes:** single-line config tweaks (e.g. budget constants, cycle-limit bumps), pure docstring/comment updates, prompt text polish, mechanical lint fixes. In auto mode, `task X approve` past gates without orchestrator review is fine for these. In manual mode, the gate-time review is already integrated (you wrote the plan + code, you also do the diff review). If the diff is more than ~50 lines, touches more than one file, or modifies any sensitive surface, take both gate reviews seriously regardless of mode.

**Briefing the gate-time reviewer.** The reviewer reads the FULL plan front-to-back — not just the slice under review — so it can catch a choice in this commit that quietly contradicts a *later* commit's stated assumption. That independent read is the whole point of an adversarial reviewer; a reviewer that only checks what your focus list names inherits your blind spots. At the plan_approval gate, also brief with: the plan path, a list of "things the harness reviewers already covered" (from the cycle log), and explicit instruction to focus on semantic gaps rather than surface patterns. At the build_approval gate, brief with: the actual diff (`git show <commit>`), the plan, and the same "already covered" list. Frame that list + the focus areas as POINTERS — where cheap coverage already exists, so the reviewer spends its budget on the gaps — NOT fences: it stays free to flag anything its own read surfaces. Tell the reviewer where to *not spend* its context, never what it's forbidden to look at.
...
**The gate review is strongest as a multi-perspective adversarial pass, not a single read.** A solo read of a diff shares the author's blind spots — you reason about the change the way the Coder did, so you both miss the same edge. Independent skeptics each on a distinct dimension (for a fix: correctness / the-changed-invariant's-safety / the negative paths; for a migration: schema-fidelity / data-preservation / atomicity) surface what one read doesn't — that is what an adversarial review *workflow* (N reviewers + a synthesis) buys. Not theater: a solo read can clear a review-halt fix as "negative paths preserved" while a multi-dimension review catches a reachable bug the solo framing structurally couldn't see — e.g. a prior *completed* review cycle masking a *later* halt. Run the multi-perspective pass for any sensitive surface; one sharp reviewer suffices for a clean small change.

**Stale-plan check before dispatching aged tasks.** If a task sat for a while between planning and building, the codebase may have moved underneath the plan. Before `move build`:
...
- **Manual mode:** edit the plan file directly (it's just a file on disk) to refresh against current code, then run `task X review run --follow` to validate the refresh. Cheaper than auto-mode re-planning since you control the diff.
- **Auto mode:** `task X reject --feedback "refresh against current code"` from a review gate to force a new planner cycle that incorporates fresh context.
