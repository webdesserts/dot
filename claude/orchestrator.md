...
**Declare dependencies explicitly.** When tickets or subtasks have an ordering — one builds on another's merged code or convention, or simply needs to ship first — proactively use the `dependencies` command (`task <id> dependencies add <ref>`, or `--deps` at `subtask add`) to declare and track it rather than holding the order only in your head. A declared dependency hard-blocks `move` until the blocker is `Done`, so the ordering is visible in the task graph and survives context loss instead of living implicitly in your dispatch sequence.
...
**Mode selection (post-2026-05-19 arc).** Tasks have a mode (`auto | manual`) set at `task create --mode ...`, or globally via `task mode auto|manual`, or per-task via `task <id> mode`. Pick by who's writing plan/code:
...
**Subtask decomposition — manual parent / auto subtasks (validated 2026-06-02, #84 dogfood).** For work worth decomposing, the high-leverage shape is a MANUAL parent with AUTO subtasks: you author the decomposition + the subtasks (`task <id> subtask add --title … --deps <proj>#<root>.<seq>`), gate each subtask's plan and build, and merge; each AUTO subtask runs the cheap local model (qwen) to plan/build/self-review its own slice — the expensive model judges, the cheap model writes. Crux: **child dispatch is unconditional, not mode-gated** — `move plan`/`move build` on a subtask IS the dispatch trigger and fires regardless of the parent's mode. A subtask inherits the parent's mode at creation (a manual parent yields manual-labelled children); they must run AUTO — the harness now resets a child to Auto at creation, but if one still shows manual, `task <id>.<seq> mode auto` before `move`. Close the decomposition parent (whose own plan/build never ran) with `task <parent> move done --force` once its subtasks are done — `--force` lands `Done(Complete)` from any non-terminal phase, so the parent's untouched plan/build is no obstacle. **Poll subtasks; don't trust silence** — a harness/qwen subtask runs in the daemon, not your session, and its completion does NOT re-invoke you; poll `task <id> status` + the subtask's `events.jsonl` freshness on a cadence to tell working from hung.
