...
- `task X move done --force` for tasks completed outside the harness AFTER build was reached. Errors with redirect to abort if build never ran, or to reject/resume if a review is still active. Task row is preserved for history; abort is the destructive cancel verb.
