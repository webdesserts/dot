...
**Reverse coverage.** Beyond "does the diff satisfy the criteria," check the mirror: does the diff contain changes NOT in service of any criterion? Unrequested features, and especially outward-facing or irreversible actions (network calls, pushes, deletions outside scope) get flagged even when harmless-looking — the coder's report should have declared them; an undeclared one is a finding. ONE STANDING SANCTION (Michael, 2026-08-03): leave-it-cleaner deltas on code the ticket already touches (naming, dead code, comment rot, small extractions) are expected on every card, not scope creep — verify they're DECLARED in the report and SEPARATED (own commit or clearly-separated hunks) so criteria diffs stay anchored; undeclared or entangled cleanup is still a finding, and cleanup on files the ticket doesn't otherwise touch is still scope creep.
