...
- Keep changes focused on the prompt's scope. Don't add features beyond what was asked. But cleanup of code you're already touching is standing scope on every card (Michael, 2026-08-03): leave touched code cleaner than you found it, even if only minutely — naming, dead code, comment rot, small extractions. It doesn't matter who left the mess. Keep cleanup SEPARATED (own commit or clearly-separated hunks) and DECLARED in your report so reviews stay anchored; cleanup on files the ticket doesn't otherwise touch stays out.
- Don't add error handling for scenarios that can't happen. Don't create abstractions for one-time operations.
- **Watch file size.** If a file you're modifying exceeds ~1000 lines (or your changes would push it past), flag it as a refactor / DRY-up / split candidate in your report. Large files burn read budget and make changes harder to land cleanly. Don't refactor inline — the Orchestrator decides whether to split now or defer. Past ~1000 lines, the leave-it-cleaner duty (above) sharpens toward extraction: prefer cleanup that MOVES a coherent block out over cosmetic tidying. At ~2000 lines the register changes (Michael's standing rule, restated 2026-08-03: "at 1k I start considering splitting things out, at 2k I stop what I'm doing and refactor"): work a real reduction into the card you're on — the file should be smaller when you leave than when you arrived — and report it as an ESCALATION so the Orchestrator cards the remainder as immediate next work. Michael's counterfactual is the whole rationale: shrinking a file 3% on every touch instead of growing it 3% means never needing a big-bang refactor (a 14.8k-line test file once survived three arcs of polite mentions). A full-file restructure mid-ticket still coordinates with the Orchestrator first — reduction is standing scope, wholesale reorganization is not.
- **Anchor to the destination, not the journey.** Comments describe what the code does now, not what it used to do. Tests assert what the code should produce, not what today's specific bug produced. The journey belongs in commit messages; in-code comments and test assertions rot when anchored to it. When a journey narrative IS worth keeping (a real incident whose story helps future maintainers), its home is the TEST guarding that behavior — an incident-report header (Michael's report → root cause → fix), never prod code (Michael's ruling, 2026-08-03): prod comments state the constraint that still binds; the war story lives next to the sensor that guards it. When a historical reference is genuinely needed, point at the DURABLE layer (Michael, 2026-08-03): problem keys preferred, an observation key only when no problem exists yet (repoint once one does) — and no board reference at all when the constraint statement stands on its own, which is the preferred state.
