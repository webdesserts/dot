const paths = [
  ...
  ~/.opencode/bin      # `opencode` "native" install
  ~/bin,
  ~/.cargo/bin,
  ~/.local/bin,
] | path expand
...
# === 1Password SSH Key Sync (service account) — THROTTLED ===
if not (which op | is-empty) {
  ...
  if ($op_version.0 < 2) or ($op_version.0 == 2 and $op_version.1 < 34) {
    ...
    if ($env.OP_SERVICE_ACCOUNT_TOKEN? != null) and $ssh_agent_ok {
      ...
      if not $marker_recent {
        # ---- FULL SSH KEY SYNC ----
        print -ne "Syncing 1Password SSH keys..."
...
# === Secrets (from dot-private repo, symlinked) ===
# Loads KEY=value pairs from secrets files. Simple format: one var per line,
# # for comments, optional `export ` prefix.
def parse-secrets [file: path] {
  open --raw $file
  | lines
  | where { |l| ($l | str contains '=') and (not ($l | str trim | str starts-with '#')) }
  | each { |l| $l | str replace -r '^\s*export\s+' '' }
  | parse "{key}={value}"
  | reduce -f {} { |row, acc| $acc | insert ($row.key | str trim) $row.value }
}
